/**
 * UZ Tech - Webinar Digital Product Passport (9 luglio) - Iscrizioni Telesales
 ...
 *   5) Incolla quell'URL in config.js -> UZTECH_CONFIG.ENDPOINT
 *
 * Ad ogni iscrizione viene aggiunta una riga nel foglio, una colonna per campo.
 */

var SHEET_NAME = 'Iscrizioni';
var SPREADSHEET_TITLE = 'UZ Tech DPP 09-07 - Iscrizioni Telesales';

var HEADERS = [
  'Timestamp', 'Nome', 'Cognome', 'Email', 'Prefisso', 'Telefono',
  'Azienda', 'Figura professionale', 'Provincia',
  'Interesse webinar', 'Interesse Partner Kit', 'Consenso privacy', 'Fonte'
];
...
function getOrCreateSpreadsheet_() {
  ...
  if (sh.getLastRow() === 0 || sh.getRange(1, 1).getValue() !== HEADERS[0]) {
    ...
    sh.getRange(1, 1, 1, HEADERS.length)
      .setFontWeight('bold').setFontColor('#ffffff').setBackground('#0891b2');
    sh.setFrozenRows(1);
...
// health check
function doGet(e) {
  return json_({ ok: true, service: 'UZ Tech DPP iscrizioni', ts: new Date().toISOString() });
}
