...
LOG="/Users/simocors/Desktop/telesales/mon_run/fire.log"
def log(m):
...
def submit_retry(name,recs):
    ...
    for att in range(3):
        try:
            ...
            log(f"  submit retry {att+1}/3: {e}"); _t.sleep(20)
            try:
...
res=submit_retry(f"{PREFIX}-{SLOT}", recs); bid=res.get('id','?'); log(f"  batch {bid}")
...
for rnd in (1,2):
    ...
    time.sleep(60)
    res=submit_retry(f"{PREFIX}-{SLOT}-r{rnd}", [by[p] for p in failed if p in by]); bid=res.get('id','?')
log(f"  {SLOT} FATTO")
