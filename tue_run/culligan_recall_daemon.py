...
LOG="/tmp/culligan_recall_09.log"; STATE="/tmp/recall_state_09.json"
def log(m):
...
def todays_batches():
    ...
    for b in bs:
        nm=b.get("name","")
        if nm.startswith("09jun-") and not nm.startswith("recall-"):
            out.append(b.get("id"))
    return out
...
def main():
    ...
    while datetime.now()<end:
        ...
        for ph in due:
            ...
            res=submit_batch(f"09jun-recall-{ph[-4:]}", [rec])
