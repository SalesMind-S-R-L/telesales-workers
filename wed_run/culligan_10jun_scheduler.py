...
LOG="/tmp/culligan_10jun.log"
TARGET=date(2026,6,10)  # lunedì
def log(m):
...
def run_slot(name,ts,rounds=2):
    ...
    res=submit_batch(f"10jun-{name}",recs); bid=res.get('id','?'); log(f"  batch {bid}")
    ...
    for rnd in range(1,rounds+1):
        ...
        res=submit_batch(f"10jun-{name}-r{rnd}",rt); bid=res.get('id','?'); log(f"  retry {rnd}: {len(rt)} -> {bid}")
plan=json.load(open('/Users/simocors/Desktop/telesales/wed_run/plan_10jun.json'))
SCHED=[('10:00','slot_1000'),('11:30','slot_1130'),('15:00','slot_1500'),('17:30','slot_1730')]
log(f"=== SCHEDULER MERCOLEDI 10/06 armato (target {TARGET}) ===")
