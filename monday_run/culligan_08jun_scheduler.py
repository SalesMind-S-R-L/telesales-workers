...
LOG="/tmp/culligan_08jun.log"
TARGET=date(2026,6,8)  # lunedì
def log(m):
...
def run_slot(name,ts,rounds=2):
    ...
    res=submit_batch(f"08jun-{name}",recs); bid=res.get('id','?'); log(f"  batch {bid}")
    ...
    for rnd in range(1,rounds+1):
        ...
        res=submit_batch(f"08jun-{name}-r{rnd}",rt); bid=res.get('id','?'); log(f"  retry {rnd}: {len(rt)} -> {bid}")
plan=json.load(open('/Users/simocors/Desktop/telesales/monday_run/plan_08jun.json'))
SCHED=[('10:00','slot_1000'),('11:30','slot_1130'),('15:00','slot_1500'),('16:30','slot_1630')]
log(f"=== SCHEDULER LUNEDI 08/06 armato (target {TARGET}) ===")
